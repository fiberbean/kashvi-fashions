...
export default function ProductDetailPage() {
    ...
    async function fetchProductAndInventory() {
      ...
      try {
        ...
        if (data) {
          ...
          // Fetch Inventory Stock where stock_quantity > 0
  ...
  // Filter sizes dynamically based on selected color's inventory stock
  const availableSizesForSelectedColor = selectedColor
    ? Array.from(
        new Set(
          inventoryStock
            .filter((item: any) => item.variant_color === selectedColor)
            .map((item: any) => item.variant_size)
            .filter(Boolean)
        )
      )
  ...
  const handleColorClick = (colorName: string) => {
    ...
    // Automatically reset size to the first available size for this color
    const sizes = inventoryStock
      .filter((item: any) => item.variant_color === colorName)
      .map((item: any) => item.variant_size)
      .filter(Boolean);
