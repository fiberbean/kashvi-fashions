...
// --- SUB-COMPONENT 2: COMPACT REALTIME DASHBOARD ---
function AdminDashboardView({
  ...
  const fetchDashboardData = async (silent = false) => {
    try {
      ...
      // Abandoned carts fallback/fetch
      ...
      console.error('Error in fetching dashboard state:', err);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    // 1. WebSocket Live Order Subscription
    ...
    // 2. Automatic Polling Interval every 15 seconds (Auto Sync)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-12 select-none">
      ...
      {/* 1. Header Bar with Auto Sync Indicator */}
      ...
      {/* 2. Compact Bento Cards (Clean, pleasant & modern fonts) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* CARD 1: NEW ORDERS RECEIVED */}
      ...
      {/* 3. Main Data Tabs (Live Orders & Cart Recovery) */}
      <div className="bg-white rounded-2xl border border-[#e2eae6] shadow-xs overflow-hidden">
        ...
        {/* TAB 2: CART ABANDONED USERS PANEL */}
        {tableTab === 'abandoned' && (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-sans">
              <thead className="bg-[#fbfcfc] text-[#809c93] uppercase text-[9.5px] font-bold tracking-wider border-b border-[#edf2ef]">
...
// --- SUB-COMPONENT 3: PERSISTENT SLIDE NOTIFICATIONS CONTAINER ---
