  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        setImages((prev) => [
          ...prev,
          { id: `${Date.now()}_${i}`, url: base64Url, color_tag: defaultTag }
        ]);
      }
    } finally {
      ...
      <div className="bg-white rounded-3xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-[#dce6e1] space-y-4 text-xs">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="p-3.5 bg-[#f8faf9] rounded-2xl border border-[#dce6e1] space-y-3">
            <span className="text-xs font-bold text-[#0b3b2c] block uppercase tracking-wider">Product Images & Colour Tagging</span>
            <input
              ref={fileInputRef}
              type="file"
              multiple
            ...
            <label
              htmlFor="prod-file"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-[#dce6e1] text-[#0b3b2c] font-bold text-xs cursor-pointer"
            >
              <Upload className="w-4 h-4 text-[#e5c07b]" />
              <span>{uploadingImage ? 'Uploading...' : 'Choose Product Images'}</span>
            </label>

            {images.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                {images.map((img) => (
                  <div key={img.id} className="rounded-2xl border border-[#dce6e1] bg-white overflow-hidden">
