...
export default function PurchaseManager() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Alphanumeric Sort (KF0001, KF0002, KF0003...)
      if (prodRes.data) {
  ...
  const handleDeletePurchase = async (p: PurchaseRecord) => {
    const confirmDelete = window.confirm(`Permanently delete [${p.id}]? Ee bill loni stock inventory nunchi rollback avthundi.`);
  ...
  // Search in Products by Code or Name
  ...
  // LEFT TO RIGHT PUSH: Add Staged Matrix items to Right Side Queue
  ...
  return (
    <div className="space-y-4 font-sans text-sm select-none">
      ...
      {/* 1. Header Bar */}
      <div className="p-4 rounded-2xl bg-[#101628]/95 border border-white/10 shadow-xl flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-extrabold text-white tracking-tight flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-[#00ff9d]/20 text-[#00ff9d] border border-[#00ff9d]/40 text-xs font-mono">
                {purchases.length} Invoices
              </span>
            </h2>
            <span className="text-xs text-[#8b9bb4]">
              Left-Right Split Layout • Alphanumeric Product Code Search • Color & Size Matrix Desk
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-64 sm:w-80">
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-[#0a0e17] rounded-xl text-white text-xs outline-none border border-white/10 focus:border-[#ffa500]"
            />
            <Search className="w-4 h-4 text-[#8b9bb4] absolute left-3 top-1/2 -translate-y-1/2" />
          </div>

          <button
            type="button"
            onClick={loadData}
            className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-[#00d9ff] cursor-pointer"
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#ffa500] to-[#ff6b6b] hover:opacity-95 text-white font-bold text-xs flex items-center gap-1.5 shadow-lg shadow-[#ffa500]/30 cursor-pointer active:scale-95"
          >
      ...
      <div className="rounded-2xl bg-[#101628]/95 border border-white/10 shadow-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-[#0a0e17]/80 text-[#8b9bb4] font-mono text-xs uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-white/5 text-xs">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="p-8 text-center text-[#8b9bb4] italic text-sm">
                ...
                filteredPurchases.map((p) => (
                  <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="p-3.5 font-mono font-extrabold text-[#00ff9d] text-sm">
                      {p.id}
                    </td>
                    ...
                    <td className="p-3.5">
                      ...
                      <span className="text-[10.5px] font-mono text-[#8b9bb4]">{p.supplier_bill_date || ''}</span>
                    </td>
      ...
      {/* 3. WIDE LEFT-RIGHT SPLIT MODAL (COMFORTABLE FEEDING DESK) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[99999] pt-12 pb-4 px-2 sm:px-4 flex items-start justify-center bg-black/90 backdrop-blur-xl overflow-y-auto select-none">
          <div className="bg-[#101628] border border-white/20 rounded-3xl w-full max-w-[98vw] xl:max-w-7xl overflow-hidden shadow-2xl relative flex flex-col my-auto max-h-[92vh]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-white flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-[#ffa500]/20 text-[#ffa500] border border-[#ffa500]/40 text-[10px] font-mono">
                      LEFT: CREATION • RIGHT: INWARD QUEUE
                    </span>
                  </h3>
                  <span className="text-xs text-[#8b9bb4]">
                    Configure matrix on Left, add to Right side queue, and verify Grand Total
                  </span>
                </div>
              </div>

              <div className="flex items-center gap-3">
                <div className="bg-[#101628] px-3.5 py-1.5 rounded-xl border border-white/15 text-right font-mono text-sm font-extrabold text-[#00ff9d]">
                  {purchaseCodeLoading ? <Loader2 className="w-4 h-4 animate-spin text-[#00d9ff] ml-auto" /> : purchaseNo}
                </div>
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="p-2 rounded-xl bg-white/10 hover:bg-[#ff6b6b]/30 text-[#8b9bb4] hover:text-white cursor-pointer transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <form onSubmit={handleSavePurchase} className="p-4 sm:p-5 overflow-y-auto space-y-3.5 custom-scrollbar text-xs">
              
              {/* Master Header: Supplier, Bill No, Dates */}
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 p-3.5 rounded-2xl bg-[#0a0e17]/90 border border-white/10">
                <div>
                  <label className="text-[10.5px] font-mono text-[#8b9bb4] uppercase block mb-1 font-bold">
                    Supplier Name *
                  </label>
                  <select
                    required
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-[#101628] border border-white/15 text-white font-semibold outline-none text-xs focus:border-[#ffa500]"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-[#101628] border border-white/15 text-[#00ff9d] font-mono font-bold outline-none text-xs"
                  />
                </div>
              </div>

              {/* TWO-COLUMN SPLIT PANEL: LEFT (CREATION & MATRIX) vs RIGHT (QUEUE & SAVED ITEMS) */}
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
                ...
                <div className="lg:col-span-7 space-y-3">
                  ...
                  {editingPurchase && !isEditProductUnlocked ? (
                    <div className="p-4 rounded-2xl bg-[#6d4aff]/10 border border-[#6d4aff]/30 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#6d4aff]/20 text-[#00d9ff] flex items-center justify-center">
                          <Lock className="w-5 h-5" />
                        </div>
                        <div>
                          <span className="font-bold text-white text-sm block">Add Products to Saved Purchase Bill</span>
                          <span className="text-xs text-[#8b9bb4]">Protected by Order Pipeline Security PIN verification</span>
                        </div>
                      </div>
                    ...
                    <div className="p-4 rounded-2xl bg-[#0a0e17] border border-white/10 space-y-3">
                      
                      {/* Product Selector with Alphanumeric Sorting & Live Filter */}
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-mono font-bold text-[#00d9ff] uppercase flex items-center gap-2">
                          <Layers className="w-4 h-4" /> 1. Search & Select Product (Code / Name)
                          {editingPurchase && (
                            <span className="ml-2 px-2 py-0.5 rounded-full bg-[#00ff9d]/20 text-[#00ff9d] text-[9px] font-mono font-bold flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="px-3 py-1 rounded-xl bg-[#6d4aff]/20 border border-[#6d4aff]/40 text-[#00d9ff] hover:text-white text-xs font-bold"
                        >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                        ...
                        <div className="sm:col-span-8 relative" ref={dropdownRef}>
                          <div
                            ...
                            className="w-full px-3.5 py-2.5 rounded-xl bg-[#101628] border border-white/20 text-white font-semibold text-xs flex items-center justify-between cursor-pointer focus-within:border-[#00d9ff]"
                          >
                            <span className={activeProduct ? 'text-white font-bold' : 'text-[#8b9bb4]'}>
                              {activeProduct ? `[${activeProduct.id}] ${activeProduct.name}` : 'Search Product Code (KF...) or Name...'}
                            </span>
                            <ChevronDown className="w-4 h-4 text-[#8b9bb4]" />
                          </div>

                          {isProductDropdownOpen && (
                            <div className="absolute left-0 right-0 top-full mt-1.5 z-50 bg-[#101628] border border-white/20 rounded-2xl shadow-2xl p-2 max-h-72 overflow-hidden flex flex-col">
                              <div className="relative mb-2">
                                <input
                                  type="text"
                                  autoFocus
                                  placeholder="Type product code (KF...) or name..."
                                  value={productSearchTerm}
                                  onChange={(e) => setProductSearchTerm(e.target.value)}
                                  className="w-full pl-8 pr-3 py-1.5 rounded-xl bg-[#0a0e17] border border-white/10 text-white text-xs outline-none focus:border-[#00d9ff]"
                                />
                              ...
                              <div className="overflow-y-auto space-y-1 custom-scrollbar">
                                {filteredProductsDropdown.length === 0 ? (
                                  <div className="p-3 text-center text-[#8b9bb4] text-xs italic">
                                  ...
                                  filteredProductsDropdown.map((p) => (
                                    <div
                                      ...
                                      className="p-2 rounded-xl hover:bg-white/10 cursor-pointer flex items-center justify-between transition-colors"
                                    >
                        ...
                        <div className="sm:col-span-4">
                          <input
                            ...
                            min="0"
                            step="any"
                            placeholder="Cost Price (CP Rate) *"
                            value={unitCost}
                            onChange={(e) => setUnitCost(e.target.value)}
                            className="w-full px-3.5 py-2.5 rounded-xl bg-[#101628] border border-white/20 text-[#00ff9d] font-bold outline-none text-xs focus:border-[#00ff9d]"
                          />
                        </div>
                      </div>
                      ...
                      {activeProduct ? (
                        <div className="space-y-3 pt-2 border-t border-white/5">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <select
                              ...
                              className="px-3 py-2 rounded-xl bg-[#101628] border border-white/15 text-white text-xs outline-none focus:border-[#00d9ff]"
                            >
                            ...
                            <input
                              ...
                              className="px-3 py-2 rounded-xl bg-[#101628] border border-white/15 text-white text-xs outline-none focus:border-[#00d9ff] max-w-[150px]"
                            />

                            <button
                              type="button"
                              onClick={handleAddColorToMatrix}
                              className="px-3.5 py-2 rounded-xl bg-[#6d4aff] hover:bg-[#5b3adb] text-white font-bold text-xs cursor-pointer shadow"
                            >
                              + Add Color
                            </button>
                          ...
                          {activeMatrixColors.length > 0 ? (
                            <div className="space-y-2 pt-1">
                              <span className="text-xs font-mono font-bold text-[#8b9bb4] uppercase block">
                                3. Enter Inward Quantities
                              </span>

                              <div className="border border-white/10 rounded-2xl overflow-x-auto bg-[#101628] max-h-56">
                                <table className="w-full text-center border-collapse">
                                  <thead>
                                    <tr className="bg-[#0a0e17] text-[#8b9bb4] font-mono text-xs uppercase border-b border-white/10 sticky top-0">
                                      <th className="p-2.5 text-left min-w-[110px]">Colour \ Size</th>
                                      {productSizes.map((sz) => (
                                        <th key={sz} className="p-2.5 text-center text-[#00d9ff] min-w-[60px]">
                                          {sz}
                                        </th>
                                      ))}
                                    </tr>
                                  ...
                                  <tbody className="divide-y divide-white/5">
                                    {activeMatrixColors.map((clr) => (
                                      <tr key={clr}>
                                        <td className="p-2 text-left font-bold text-white text-xs whitespace-nowrap">
                                          {clr}
                                        </td>
                                        {productSizes.map((sz) => {
                                          const key = `${clr}:::${sz}`;
                                          return (
                                            <td key={sz} className="p-1.5 text-center">
                                              <input
                                                ...
                                                className="w-13 px-2 py-1 text-center font-mono font-bold bg-[#0a0e17] text-[#00ff9d] border border-white/10 rounded-xl outline-none text-xs focus:border-[#00ff9d]"
                                              />
                                            </td>
                                          );
                                        })}
                        ...
                        <div className="p-4 rounded-xl bg-[#101628] border border-dashed border-white/10 text-center text-[#8b9bb4] italic text-xs">
                          Choose a product from the dropdown above to start configuring its color and size matrix.
                        </div>
                      )}
                ...
                <div className="lg:col-span-5 space-y-3">
                  <div className="border border-white/10 rounded-2xl overflow-hidden bg-[#0a0e17] shadow-xl flex flex-col">
                    ...
                    {/* Header */}
                    <div className="px-4 py-3 bg-[#101628] border-b border-white/10 flex items-center justify-between text-xs font-mono">
                      <span className="font-bold text-[#00ff9d] uppercase flex items-center gap-2">
                    ...
                    {/* Staged Items List */}
                    <div className="max-h-72 min-h-[180px] overflow-y-auto custom-scrollbar p-1.5">
                      {stagedItems.length === 0 ? (
                        <div className="p-8 text-center text-[#8b9bb4] italic text-xs space-y-1.5">
                          <Layers className="w-6 h-6 mx-auto text-white/20" />
                          <p className="font-semibold text-white/60">Inward Queue is Empty.</p>
                          <p className="text-[11px] text-white/40">Select product & fill quantities on the Left, then click &quot;Add to Matrix Queue ➔&quot;.</p>
                        </div>
                      ) : (
                          ...
                          <tbody className="divide-y divide-white/5">
                            {stagedItems.map((it, idx) => (
                              <tr key={idx} className="hover:bg-white/[0.02]">
                                ...
                                <td className="p-2 text-center font-bold text-[#00ff9d]">{it.quantity}</td>
                                <td className="p-2 text-right font-mono text-[#8b9bb4]">₹{it.unit_cost}</td>
                    ...
                    {editingPurchase && existingItems.length > 0 && (
                      <div className="p-3 bg-[#101628]/60 border-t border-white/10 space-y-1">
                    ...
                    <div className="p-3.5 bg-[#101628] border-t border-white/10 space-y-2.5">
                      <div className="space-y-1.5 font-mono text-xs">
                        ...
                        <div className="flex justify-between items-center text-sm font-bold pt-1.5 border-t border-white/5">
                      ...
                      <div>
                        <input
                          ...
                          className="w-full px-3 py-2 rounded-xl bg-[#0a0e17] border border-white/10 text-white text-xs outline-none"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-2.5 pt-1">
                        ...
                        <button
                          type="submit"
                          disabled={submitting}
                          className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#ffa500] to-[#ff6b6b] hover:opacity-95 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-[#ffa500]/30 cursor-pointer active:scale-95 disabled:opacity-50"
                        >
      ...
      {isPinModalOpen && (
        <div className="fixed inset-0 z-[100000] p-3 flex items-center justify-center bg-black/85 backdrop-blur-md animate-in fade-in">
          <div className="bg-[#101628] border border-white/20 rounded-3xl max-w-sm w-full p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-[#6d4aff]/20 text-[#00d9ff] flex items-center justify-center">
                  <ShieldCheck className="w-4 h-4" />
                </div>
                <h4 className="text-sm font-bold text-white">Security Verification</h4>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleVerifySecurityPin} className="space-y-4">
              ...
              {pinError && (
                <div className="p-2 rounded-xl bg-[#ff6b6b]/15 border border-[#ff6b6b]/30 text-[#ff6b6b] text-xs font-bold text-center">
                  {pinError}
                </div>
              )}
              ...
              <input
                ...
                className="w-full text-center px-4 py-2.5 rounded-xl bg-[#0a0e17] border border-white/15 text-[#00ff9d] text-lg font-mono font-bold tracking-widest outline-none focus:border-[#00d9ff]"
              />
              ...
              <div className="flex justify-end gap-2.5 pt-1">
                <button
                  type="button"
                  onClick={() => setIsPinModalOpen(false)}
                  className="px-4 py-1.5 rounded-xl text-[#8b9bb4] hover:text-white text-xs"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#6d4aff] to-[#00d9ff] text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-[#6d4aff]/30 cursor-pointer"
                >
      ...
      {/* 5. VIEW PURCHASE DETAIL MODAL */}
      {viewingPurchase && (
        <div className="fixed inset-0 z-[99999] p-3 flex items-center justify-center bg-black/85 backdrop-blur-md animate-in fade-in">
          <div className="bg-[#101628] border border-white/15 rounded-3xl max-w-xl w-full overflow-hidden shadow-2xl p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div>
                <span className="font-mono text-sm font-extrabold text-[#00ff9d]">{viewingPurchase.id}</span>
              ...
              <button
                ...
                className="p-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-[#8b9bb4] hover:text-white"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar">
              {loadingItems ? (
                <div className="p-6 text-center text-[#8b9bb4]">
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {viewingItems.map((item) => (
                      <tr key={item.id}>
                        <td className="p-2 text-white font-semibold">{item.product_id}</td>
                        <td className="p-2 text-[#00d9ff]">{item.variant_color} / {item.variant_size}</td>
                        <td className="p-2 text-center font-bold text-[#00ff9d]">{item.quantity}</td>
                        <td className="p-2 text-right font-mono text-[#8b9bb4]">₹{item.unit_cost}</td>
                        <td className="p-2 text-right font-mono font-bold text-white">₹{item.total_cost}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
